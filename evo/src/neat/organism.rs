...
/// An organism is a NeatGenome with fitness.
...
impl NeatNetwork {
    ...
    pub fn activate(&mut self, sensors: Vec<f64>, outputs: &mut Vec<f64>) -> bool {
    ...
    pub fn run(&mut self, sensors: Vec<f64>, inputs: usize) -> Option<Vec<f64>> {
        let mut out: Vec<f64> = Vec::<f64>::fill(inputs, || 0.0);
        if self.activate(sensors, &mut out) {
            Some(out)
        } else {
            None
        }
...
mod tests {
    ...
    fn should_propagate_signal_without_hidden_layers() {
        let mut organism = NeatNetwork::new(NeatGenome::default());
